...
compact_results <- fread('mens-machine-learning-competition-2019/Stage2DataFiles/RegularSeasonCompactResults.csv')
conference_results <- fread('mens-machine-learning-competition-2019/Stage2DataFiles/ConferenceTourneyGames.csv')
setnames(conference_results, tolower(colnames(conference_results)))
setnames(compact_results, tolower(colnames(compact_results)))
tourney_results <- fread('mens-machine-learning-competition-2019/Stage2DataFiles/NCAATourneyCompactResults.csv')
setnames(tourney_results, tolower(colnames(tourney_results)))
nit_results <- fread('mens-machine-learning-competition-2019/Stage2DataFiles/SecondaryTourneyCompactResults.csv')
setnames(nit_results, tolower(colnames(nit_results)))
team_spellings <- fread('mens-machine-learning-competition-2019/Stage2DataFiles/TeamSpellings.csv')
...
test_set <- merge(bind_rows(seasons[18:34]),
                  bind_rows(rf_list),
...
submission <- fread('mens-machine-learning-competition-2019/SampleSubmissionStage2.csv')
submission[, team1 := as.numeric(substr(ID, 6, 9))]
submission[, team2 := as.numeric(substr(ID, 11, 14))]
submission_data <- merge(submission,
                         filter(final_metrics,
                                season == 2019),
                         by = 'team1')
final_submission_data <- merge(submission_data,
                               filter(final_metrics,
                                      season == 2019),
                               by.x = 'team2',
                               by.y = 'team1')
final_submission_data[, pred1 := predict(final_model,
                                         newdata = final_submission_data,
                                         type = 'response')]
final_submission_data[, pred2 := predict(final_rf,
                                         newdata = final_submission_data,
                                         type = 'prob')[,2]]
final_submission_data[, pred_a := o*pred1 + (1-o)*pred2]
final_submission_data[, pred_b := (1-o)*pred1 + o*pred2]
submission1 <- select(final_submission_data,
                      ID,
                      Pred = pred_a)
submission2 <- select(final_submission_data,
                      ID,
                      Pred = pred_b)

write.csv(submission1,
          'octo-lana_log_heavy.csv',
          row.names = F)
write.csv(submission2,
          'octo-lana_rf_heavy.csv',
          row.names = F)

library(kaggleNCAA)

dat <- parseBracket('octo-lana_log_heavy.csv', w=0)  # w=0 for men
sim <- simTourney(dat, 10023, progress=TRUE, w=0)  # w=0 for men
bracket <- extractBracket(sim)
printableBracket(bracket)

dat2 <- parseBracket('octo-lana_rf_heavy.csv', w=0)  # w=0 for men
sim2 <- simTourney(dat2, 10023, progress=TRUE, w=0)  # w=0 for men
bracket2 <- extractBracket(sim2)
